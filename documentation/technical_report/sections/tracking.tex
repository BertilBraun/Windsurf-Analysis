...
panels over grey water, spray, and haze.  Three vertical bands preserve coarse color
...
smoothness gate identifies water-like pixels, and only background components connected
to the crop border are removed.  Morphological cleanup and a minimum-mask fallback
avoid empty descriptors.  This foreground mask is important: an earlier inactive code
path used a fixed centre rectangle, allowing water and overlapping sails to dilute
the intended cue.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{tracking-sail-color-similarity.png}
  \caption{Production descriptor on consecutive crops of two sails.  Repeated
  observations receive high match scores, while the cross-sail entries reach the
  lower clip bound.  The hand-selected example explains the cue; it is not an
  accuracy result.}
...
frames most often contaminated during a crossing.  Given descriptor distance $d$, the
current heuristic match score is
\begin{equation}
 p_a=\operatorname{clip}_{[10^{-6},1-10^{-6}]}(1-11.630d).
 \label{eq:appearance}
\end{equation}
It is a tuned association score, not a calibrated posterior probability.
...
\begin{equation}
 ...
 \label{eq:edge-cost}
\end{equation}
Motion is the mean negative log score from the first two observations of $T_j$;
appearance uses the whole-fragment prototypes; and the gap term increases with the
number of missing frames.

The graph now applies absolute plausibility vetoes before optimization.  Edges are
removed when motion NLL exceeds 8, or when endpoint box area or aspect ratio changes
...
\begin{figure}[t]
  ...
  \caption{Complete-video evidence supports global tracklet association.  Later
