...
def cut_worker_inner(item, done_list):
    ...
    try:
        if do_cut:
            ...
            cmd_cut = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-ss', str(seg['start'])]
...
def annotate_and_cut(input_path: Path, out_dir: Path, n_workers: int, no_input: bool = False):
    ...
    workers = [
        threading.Thread(target=cut_worker, args=(segment_queue, done_segments), daemon=True) for _ in range(n_workers)
    ]
    for w in workers:
        w.start()
    ...
    for i, vid in enumerate(videos):
        ...
        if no_input:
            ...
            seg = {'source': str(vid), 'start': 0, 'end': None, 'rotate': 0, 'stabilize': g_apply_stabilized}
...
def main():
    ...
    parser.add_argument(
        '--keep-cut',
        action='store_true',
        help='Output pre-stabilized (raw cut) clip with _no_stabilize suffix even if stabilization is applied.',
    )
    parser.add_argument(
        '--no-input', action='store_true', help='Skip annotation and mpv; just process/copy/stabilize all video files.'
    )
