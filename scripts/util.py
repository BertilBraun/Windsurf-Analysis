...
def stabilize_ffmpeg(input_file: os.PathLike, output_file: os.PathLike) -> bool:
    ...
    cmd_detect = [
        ...
        "-i", input_file, "-vf", f"vidstabdetect=shakiness=5:accuracy=15:result={trf_file}",
        "-f", "null", "-", "-y"
    ]
    cmd_stab = [
        "ffmpeg", "-hide_banner", "-loglevel", "error",
        "-i", input_file, "-vf", f"vidstabtransform=smoothing=30:input={trf_file},unsharp=5:5:0.8:3:3:0.4",
        "-c:v", "libx264", "-c:a", "copy", output_file, "-y"
    ]
    try:
