#!/usr/bin/env python3
"""
quick_annotate.py – minimal multi-box annotator with grow/shrink toggle
...
def redraw(base, bx_list, highlight_last=True):
    ...
    for i, bx in enumerate(bx_list):
        ...
        if highlight_last and i == n-1 and not drawing:
            ...
            thickness = 3
        cv2.rectangle(disp, (int(bx[0]), int(bx[1])),
                             (int(bx[2]), int(bx[3])), color, thickness)
...
while sample_count < args.samples:
    ...
    while True:
        cv2.setWindowTitle("annotate", f"annotate [{sample_count}/{args.samples}] {vpath.name} (frame {frame_no+1}/{fcnt})")
        ...
        # Fine-tune last box
        elif key == ord('w'):      adjust_last(dy1=-1)
        elif key == ord('a'):      adjust_last(dx1=-1)
        elif key == ord('s'):      adjust_last(dy2=1)
        elif key == ord('d'):      adjust_last(dx2=1)
