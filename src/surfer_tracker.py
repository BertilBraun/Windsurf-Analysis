...
class SurferTracker:
    ...
    def _generate_individual_videos(
        self,
        valid_tracks: dict[int, list[Track]],
        original_video_path: os.PathLike,
        output_dir: os.PathLike | str,
    ) -> None:
        """Generate individual cropped videos for each valid track using original video resolution

        Args:
            original_video_path: Path to original high-resolution video
            valid_tracks: Dictionary of valid track data with bboxes in YOLO coordinates
            output_dir: Directory to save individual videos
        """
        if not valid_tracks:
            print('No valid tracks found for individual video generation')
            return
        ...
        print(f'Generating individual videos for {len(valid_tracks)} tracks...')
        ...
        for person_number, (track_id, track_data) in enumerate(valid_tracks.items(), 1):
            ...
            for detection in track_data:
                # Scale YOLO bbox to original video coordinates
            ...
            # Create slice size with generous context (3x bbox size)
            context_factor = 1.5
    ...
    def process_tracks(
        ...
        """Complete pipeline: greedy merge all tracks, remove overlapping tracks, filter for valid ones, smooth centers, and generate individual videos.
        ...
        Args:
            ...
            output_dir: Directory to save individual videos
        """
        # Set total frames from the original video for percentage calculation
        video_properties = get_video_properties(original_video_path)
        ...
        valid_tracks = self._get_valid_tracks(merged_tracks, video_properties.total_frames)
        ...
        print(f'After greedy merging and overlap removal: {len(valid_tracks)} tracks remaining')
        for track_id, track_data in valid_tracks.items():
        ...
        self._generate_individual_videos(smoothed_tracks, original_video_path, output_dir)
