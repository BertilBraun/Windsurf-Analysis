...
from common_types import Detection, FrameIndex, Track, cosine_similarity
...
def count_max_simultaneous_detections(detections: list[Detection]) -> int:
...
class DiscreteOptimizationTracker:
    ...
    def _optimize_fragments(
        ...
        link_cost_terms = [
            z3.If(v, z3.RealVal(str(pair_cost[(i, j)])), z3.RealVal('0')) for (i, j), v in link_vars.items()
        ]
        start_cost_terms = [z3.If(sv, z3.RealVal(str(self.w_start)), z3.RealVal('0')) for sv in start_vars]
        total_cost = (
            z3.Sum(link_cost_terms + start_cost_terms) if (link_cost_terms or start_cost_terms) else z3.RealVal('0')
        )
    ...
    def _track_detections_inner(
        ...
        assign: list[z3.ArithRef] = [z3.Int(f'track_{idx}') for idx in range(num_det)]
        for var in assign:
            opt.add(z3.And(var >= 0, var < n_tracks))

        # Frame-level AllDifferent
        for indices in frame_index_to_assignment_indices.values():
    ...
    def track_detections(
        ...
        solution_tracks = self._track_detections_inner(detections, n_tracks=n_tracks, must_link_groups=must_link_groups)
